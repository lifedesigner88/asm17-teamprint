...
- 독립 작업은 `origin/main` 최신 기준으로 새 브랜치를 만든다.
- 아직 머지되지 않은 이전 작업에 의존하는 작업은, 그 이전 작업 브랜치에서 새 브랜치를 만든다.
...
- PR은 `PR 하나 = 브랜치 하나` 원칙으로 관리한다.
- stacked branch가 필요하면 새 브랜치를 이전 작업 브랜치에서 파고, 필요할 때만 `main` 기준으로 다시 정리한다.
- PR을 새로 만들거나 기존 브랜치에 추가 푸시하기 전에는, 현재 작업 브랜치가 이미 merge 된 PR 브랜치인지 먼저 확인한다.
- 이미 merge 된 PR 브랜치에는 새 커밋을 추가하지 않는다. 필요하면 최신 기준 브랜치에서 새 브랜치를 만들고 필요한 커밋만 옮긴다.
- 새 PR을 올리기 전에는 이미 merge 된 브랜치를 정리 대상으로 확인한다.
- stacked child가 아직 부모 브랜치를 base로 쓰는 경우가 아니면, merge 된 브랜치는 로컬과 원격에서 삭제하는 것을 기본 정책으로 둔다.
