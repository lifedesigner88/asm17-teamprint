...
- PR을 새로 올리거나 기존 브랜치에 추가 푸시하기 전에는, 현재 작업 브랜치가 이미 merge 된 PR 브랜치가 아닌지 먼저 확인한다.
- 이미 merge 된 PR 브랜치에는 새 커밋을 올리지 않는다. 그런 경우 최신 기준 브랜치(`origin/main` 또는 올바른 부모 브랜치)에서 새 브랜치를 만들고 필요한 커밋만 옮긴다.
